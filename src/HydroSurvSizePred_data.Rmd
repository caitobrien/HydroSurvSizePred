...
df<-df.fish.raw %>% 
  ...
  mutate(by_half_month = ifelse(day_month <= 15, paste0("Early ", by_month), paste0("Late ", by_month))) %>%
  mutate(by_half_month = factor(by_half_month, levels = unique(by_half_month))) %>% 
